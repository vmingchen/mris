...
based on not only their hotness but also their size, so that they can
...
system presents up to 5.3$\times$ speedup over the single-tier store
using HDD. Particularly, for a workload emulating Facebook's photo
requests, we observe a sppedup of 5.3$\times$ of the throughput using
only a small Flash of 5.9\% of the total storage capacity.
